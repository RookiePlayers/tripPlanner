...
import {
  Home, Plane, Ticket, ShieldCheck, BedDouble, Map as MapIcon, Wallet, Sun, Moon, UserCircle
} from 'lucide-react'
import { GateProvider } from '../components/PasswordGate'
import protectedRoutes from '../config/auth/protectedRoutes'
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    // Server-side validation (catches disabled/deleted accounts)
    fetch('/api/auth/validate', {
      method: 'POST',
    })
      .then(res => {
        if (!res.ok) clearAuthAndRedirect()
      })
      .catch(() => {
        // Network error — don't kick the user out
      })
  }, [pathname, router, clearAuthAndRedirect])
